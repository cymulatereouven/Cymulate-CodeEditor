...
import { mountCymulateCodeEditorOnboarding } from './react/out/cymulateCodeEditor-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.cymulateCodeEditor-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountCymulateCodeEditorOnboarding(onboardingContainer, accessor);
