...
import { ProviderName, SettingName, displayInfoOfSettingName, providerNames, CymulateCodeEditorStatefulModelInfo, customSettingNamesOfProvider, RefreshableProviderName, refreshableProviderNames, displayInfoOfProviderName, nonlocalProviderNames, localProviderNames, GlobalSettingName, featureNames, displayInfoOfFeatureName, isProviderNameDisabled, FeatureName, hasDownloadButtonsOnModelsProviderNames, subTextMdOfProviderName } from '../../../../common/voidSettingsTypes.js'
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js'
import { CymulateCodeEditorButtonBgDarken, CymulateCodeEditorCustomDropdownBox, CymulateCodeEditorInputBox2, CymulateCodeEditorSimpleInputBox, CymulateCodeEditorSwitch } from '../util/inputs.js'
...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-cymulateCodeEditor-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<CymulateCodeEditorButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
			...
			</CymulateCodeEditorButtonBgDarken>
		</div>
	);
};
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('ICymulateCodeEditorSettingsService');
	...
	const sourcecodeOverridesLink = `https://github.com/cymulatereouven/Cymulate-CodeEditor/blob/2e5ecb291d33afbe4565921664fb7e183189c1c5/src/vs/workbench/contrib/void/common/modelCapabilities.ts#L146-L172`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-cymulateCodeEditor-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-cymulateCodeEditor-fg-3 hover:text-cymulateCodeEditor-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="text-sm text-cymulateCodeEditor-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with CymulateCodeEditor, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by CymulateCodeEditor.`
							: `CymulateCodeEditor recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<CymulateCodeEditorSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
					<span className="text-cymulateCodeEditor-fg-3 text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-cymulateCodeEditor-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-cymulateCodeEditor-border-2 bg-cymulateCodeEditor-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-cymulateCodeEditor-fg-3' : ''}`}
				...
				<div className="flex justify-end gap-2 mt-4">
					<CymulateCodeEditorButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</CymulateCodeEditorButtonBgDarken>
					<CymulateCodeEditorButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (CymulateCodeEditorStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='cymulateCodeEditor-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='cymulateCodeEditor-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					{disabled ? null : (
						<div className="w-5 flex items-center justify-center">
							...
							>
								<Plus size={12} className="text-cymulateCodeEditor-fg-3 opacity-50" />
							</button>
						</div>
					)}
		...
		{showCheckmark ? (
			...
			<div className="mt-4">
				<form className="flex items-center gap-2">
					...
					{/* Provider dropdown */}
					<ErrorBoundary>
						<CymulateCodeEditorCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-cymulateCodeEditor-bg-1 text-cymulateCodeEditor-fg-1 placeholder:text-cymulateCodeEditor-fg-3 border border-cymulateCodeEditor-border-2 focus:border-cymulateCodeEditor-border-1 py-1 px-2 rounded"
					...
					{/* Model name input */}
					<ErrorBoundary>
						<CymulateCodeEditorSimpleInputBox
							value={modelName}
							compact={true}
					...
					<button
						...
						className='text-cymulateCodeEditor-fg-4'
					>
						<X className='size-4' />
					</button>
				</form>
			...
			<div
				className="text-cymulateCodeEditor-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
export const CymulateCodeEditorProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	return <ButtonLeftTextRightOption
		leftButton={<CymulateCodeEditorSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
...
export const AIInstructionsBox = () => {
	...
	const voidSettingsService = accessor.get('ICymulateCodeEditorSettingsService')
	const voidSettingsState = useSettingsState()
	return <CymulateCodeEditorInputBox2
...
const FastApplyMethodDropdown = () => {
	...
	return <CymulateCodeEditorCustomDropdownBox
		className='text-xs text-cymulateCodeEditor-fg-3 bg-cymulateCodeEditor-bg-1 border border-cymulateCodeEditor-border-1 rounded p-0.5 px-1'
		options={options}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-cymulateCodeEditor-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='cymulateCodeEditor-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`CymulateCodeEditor automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const voidSettingsService = accessor.get('ICymulateCodeEditorSettingsService')
	return <div
		className={`text-cymulateCodeEditor-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	return <>
		...
		/>
		<span className="text-cymulateCodeEditor-fg-3 text-xs">{desc}</span>
	</>
}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<CymulateCodeEditorButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'cymulateCodeEditor-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(voidSettingsService.state, null, 2)
			downloadName = 'cymulateCodeEditor-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
	...
	return <div className={`@@cymulateCodeEditor-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%' }}>
		<div className='overflow-y-auto w-full h-full px-10 py-10 select-none'>

			<div className='max-w-xl mx-auto'>

				<h1 className='text-2xl w-full'>{`CymulateCodeEditor's Settings`}</h1>
				...
				<h3 className={`text-cymulateCodeEditor-fg-3 mb-2`}>{`CymulateCodeEditor can access any model that you host locally. We automatically detect your local models by default.`}</h3>
				...
				<ErrorBoundary>
					<CymulateCodeEditorProviderSettings providerNames={localProviderNames} />
				</ErrorBoundary>

				<h2 className={`text-3xl mb-2 mt-12`}>Providers</h2>
				<h3 className={`text-cymulateCodeEditor-fg-3 mb-2`}>{`CymulateCodeEditor can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
				<ErrorBoundary>
					<CymulateCodeEditorProviderSettings providerNames={nonlocalProviderNames} />
				</ErrorBoundary>
				...
				<div className='flex flex-col gap-y-8 my-4'>
					<ErrorBoundary>
						...
						<div>
							...
							<div className='text-sm italic text-cymulateCodeEditor-fg-3 mt-1'>
								<span>
									Experimental.{' '}
								</span>
								<span
									...
									data-tooltip-class-name='cymulateCodeEditor-max-w-[20px]'
								>
							...
							<div className='my-2'>
								...
								<ErrorBoundary>
									<div className='flex items-center gap-x-2 my-2'>
										...
										/>
										<span className='text-cymulateCodeEditor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
									</div>
								</ErrorBoundary>
								...
								<ErrorBoundary>
									<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
										<ModelDropdown featureName={'Autocomplete'} className='text-xs text-cymulateCodeEditor-fg-3 bg-cymulateCodeEditor-bg-1 border border-cymulateCodeEditor-border-1 rounded p-0.5 px-1' />
									</div>
								</ErrorBoundary>
					...
					<ErrorBoundary>
						...
						<div className='w-full'>
							...
							<div className='text-sm italic text-cymulateCodeEditor-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
							...
							<div className='my-2'>
								...
								<div className='flex items-center gap-x-2 my-2'>
									...
									/>
									<span className='text-cymulateCodeEditor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
								</div>

								{/* Model Dropdown */}
								<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
									<ModelDropdown featureName={'Apply'} className='text-xs text-cymulateCodeEditor-fg-3 bg-cymulateCodeEditor-bg-1 border border-cymulateCodeEditor-border-1 rounded p-0.5 px-1' />
								</div>
							</div>
					...
					<div>
						...
						<div className='text-sm italic text-cymulateCodeEditor-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
						...
						<div className='my-2'>
							...
							<ErrorBoundary>
								...
								<div className='flex items-center gap-x-2 my-2'>
									...
									/>
									<span className='text-cymulateCodeEditor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
								</div>
							</ErrorBoundary>
						</div>
					</div>
					...
					<div className='w-full'>
						...
						<div className='text-sm italic text-cymulateCodeEditor-fg-3 mt-1'>{`Settings that control the visibility of CymulateCodeEditor suggestions in the code editor.`}</div>
						...
						<div className='my-2'>
							...
							<ErrorBoundary>
								<div className='flex items-center gap-x-2 my-2'>
									...
									/>
									<span className='text-cymulateCodeEditor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
								</div>
							</ErrorBoundary>
						</div>
					</div>
				...
				<div className='mt-12'>
					<ErrorBoundary>
						...
						<h4 className='text-cymulateCodeEditor-fg-3 mb-4'>{`Transfer your editor settings into CymulateCodeEditor.`}</h4>
				...
				<div className='mt-12'>
					...
					<h4 className='text-cymulateCodeEditor-fg-3 mb-4'>{`Transfer CymulateCodeEditor's settings and chats in and out of CymulateCodeEditor.`}</h4>
					<div className='flex flex-col gap-8'>
						{/* Settings Subcategory */}
						<div className='flex flex-col gap-2 max-w-48 w-full'>
							<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
								Import Settings
							</CymulateCodeEditorButtonBgDarken>
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
								Export Settings
							</CymulateCodeEditorButtonBgDarken>
						...
						<div className='flex flex-col gap-2 w-full max-w-48'>
							...
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
								Import Chats
							</CymulateCodeEditorButtonBgDarken>
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
								Export Chats
							</CymulateCodeEditorButtonBgDarken>
				...
				<div className='mt-12'>
					...
					<h4 className={`text-cymulateCodeEditor-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

					<ErrorBoundary>
						<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
								General Settings
							</CymulateCodeEditorButtonBgDarken>
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
								Keyboard Settings
							</CymulateCodeEditorButtonBgDarken>
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
								Theme Settings
							</CymulateCodeEditorButtonBgDarken>
							<CymulateCodeEditorButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
								Open Logs
							</CymulateCodeEditorButtonBgDarken>
						</div>
					</ErrorBoundary>
				...
				<div className='mt-12 max-w-[600px]'>
					...
					<h4 className={`text-cymulateCodeEditor-fg-3 mb-4`}>
