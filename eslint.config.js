...
export default tseslint.config(
	...
	// All files (JS and TS)
	{
		...
		rules: {
			...
			'curly': 'off', // <-- CymulateCodeEditor
			'eqeqeq': 'warn',
			'prefer-const': [
				'off', // <-- CymulateCodeEditor
				{
					'destructuring': 'all'
				}
			],
			...
			'local/code-no-dangerous-type-assertions': 'off', // CymulateCodeEditor warn -> off
			...
			// CymulateCodeEditor - this should only apply to workbench/void/
			// 'header/header': [
			// 	2,
			// 	'block',
			// 	[
	...
	// TS
	{
		...
		rules: {
			'@stylistic/ts/semi': 'off', // <-- CymulateCodeEditor
			'@stylistic/ts/member-delimiter-style': 'off', // <-- CymulateCodeEditor
			...
			'jsdoc/no-types': 'warn',
			'local/code-no-static-self-ref': 'off' // <-- CymulateCodeEditor
		}
	},
	// vscode TS
	{
